...
async function initMap() {
    try {
        ...
        const [regions, demographics] = await Promise.all([
            d3.json('data/regionsinddeling_formatted_noz.geojson'),
...
async function fetchDemographicData() {
    try {
        const demographicData = await d3.dsv(';', 'data/dk_region_demo_dates_mock.csv');
...
function processDemographicData(rows) {
    ...
    rows.forEach(row => {
        ...
        const date = row.date || currentDate;
        ...
        // Store all data for this region by date
